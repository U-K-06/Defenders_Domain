...
int Game::Run()
{
  ...
  while (1)
  {
    ...
    if (elapsed_time >= GameConstants::TIME_TO_DECREASE_SPAWN_INTERVAL)
    {
      ...
      if (currentSpawnInterval >= GameConstants::MIN_SPAWN_INTERVAL)  currentSpawnInterval = GameConstants::MIN_SPAWN_INTERVAL;
    }
    ...
    for (Enemy &enemy : enemies)
    {
      ...
      if (!enemy.hasMoved)
      {
        if (elapsed_time >= 2.0f)
        {
          ...
          if ((enemy.x == door_x - 1 && enemy.y == door_y - 1) ||
              (enemy.x == door_x - 1 && enemy.y == door_y + 1) ||
              (enemy.x == door_x + 1 && enemy.y == door_y - 1) ||
              (enemy.x == door_x + 1 && enemy.y == door_y + 1))
          {
      ...
      else
      {
        ...
        {
          if (abs(dx) > abs(dy))  (dx > 0) ? enemy.x++ : enemy.x--;
          else
          {
